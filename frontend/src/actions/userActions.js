...
export const listUserStores = () => async (dispatch, getState) => {
    try {
        dispatch({
            type: USER_STORES_REQUEST
        })
        ...
        dispatch({
            type: USER_STORES_SUCCESS,
            payload: data
        })

    } catch (error) {
        dispatch({
            type: USER_STORES_FAIL,
            payload: error.response && error.response.data.message ? error.response.data.message : error.message
        })
    }
}
